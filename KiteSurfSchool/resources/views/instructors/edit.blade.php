...
<body class="bg-gradient-to-br from-blue-50 to-blue-100 min-h-screen">
    ...
    <div class="bg-gradient-to-r from-blue-600 to-blue-800 h-40 w-full relative flex items-center justify-center text-white text-xl font-semibold shadow-lg">
        ...
        <div class="absolute inset-0 bg-black bg-opacity-20 flex items-center justify-center">
            <h1 class="text-4xl font-bold text-white tracking-wider">Edit Instructor</h1>
        </div>
    </div>
    
    <div class="p-8 space-y-8 max-w-6xl mx-auto">
        <div class="bg-white p-6 rounded-xl shadow-lg hover:shadow-xl transition-shadow duration-300">
            <h2 class="text-2xl font-bold text-blue-700 mb-6">{{ __('Edit Instructor') }}</h2>
            ...
            <form method="POST" action="{{ route('instructors.update', $instructor) }}">
                ...
                <div class="mb-4">
                    <label class="block text-gray-700 text-sm font-bold mb-2">{{ __('User') }}</label>
                    <input type="text" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline bg-gray-100" value="{{ $instructor->user->name }} ({{ $instructor->user->email }})" disabled>
                </div>

                <div class="mb-4">
                    <label for="specialization" class="block text-gray-700 text-sm font-bold mb-2">{{ __('Specialization') }}</label>
                    <input id="specialization" type="text" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('specialization') border-red-500 @enderror" name="specialization" value="{{ old('specialization', $instructor->specialization) }}" required>
                ...
                <div class="mb-4">
                    <label for="bio" class="block text-gray-700 text-sm font-bold mb-2">{{ __('Bio') }}</label>
                    <textarea id="bio" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('bio') border-red-500 @enderror" name="bio" rows="4">{{ old('bio', $instructor->bio) }}</textarea>
                    @error('bio')
                        <p class="text-red-500 text-xs italic">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    <label for="certifications" class="block text-gray-700 text-sm font-bold mb-2">{{ __('Certifications') }}</label>
                    <input id="certifications" type="text" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('certifications') border-red-500 @enderror" name="certifications" value="{{ old('certifications', $instructor->certifications) }}">
                ...
                <div class="mb-6">
                    <label for="years_of_experience" class="block text-gray-700 text-sm font-bold mb-2">{{ __('Years of Experience') }}</label>
                    <input id="years_of_experience" type="number" min="0" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('years_of_experience') border-red-500 @enderror" name="years_of_experience" value="{{ old('years_of_experience', $instructor->years_of_experience) }}">
                ...
                <div class="flex items-center justify-between">
                    <a href="{{ route('instructors.show', $instructor) }}" class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">{{ __('Cancel') }}</a>
