        ...
        <div class="col-md-8">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <span>{{ __('Instructor Details') }}</span>
                    <div>
                        <a href="{{ route('instructors.edit', $instructor) }}" class="btn btn-primary btn-sm">Edit</a>
                        <a href="{{ route('instructors.index') }}" class="btn btn-secondary btn-sm">Back to List</a>
                    </div>
                </div>
                ...
                <div class="card-body">
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-md-8">{{ $instructor->user->name }}</div>
                    </div>

                    <div class="row mb-3">
                        <div class="col-md-4 fw-bold">Email:</div>
                        <div class="col-md-8">{{ $instructor->user->email }}</div>
                    </div>

                    <div class="row mb-3">
                        <div class="col-md-4 fw-bold">Specialization:</div>
                        <div class="col-md-8">{{ $instructor->specialization }}</div>
                    </div>

                    <div class="row mb-3">
                        <div class="col-md-4 fw-bold">Certifications:</div>
                        <div class="col-md-8">{{ $instructor->certifications ?? 'N/A' }}</div>
                    </div>

                    <div class="row mb-3">
                        <div class="col-md-4 fw-bold">Years of Experience:</div>
                        <div class="col-md-8">{{ $instructor->years_of_experience ?? 'N/A' }}</div>
                    </div>

                    <div class="row mb-3">
                        <div class="col-md-4 fw-bold">Bio:</div>
                        <div class="col-md-8">{{ $instructor->bio ?? 'No bio available' }}</div>
                    </div>

                    <h5 class="mt-4 mb-3">Assigned Lespakketten</h5>

                    @if ($instructor->lespakketten->count() > 0)
                        <div class="table-responsive">
                            <table class="table table-striped table-sm">
                                ...
                                <tbody>
                                    @foreach ($instructor->lespakketten as $lespakket)
                                        <tr>
                                            <td>{{ $lespakket->id }}</td>
                                            <td>{{ $lespakket->naam }}</td>
                                            <td>&euro;{{ number_format($lespakket->prijs, 2) }}</td>
                                            <td>{{ $lespakket->duur }} hours</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
                    ...
                    @else
                        <p>No lespakketten assigned to this instructor.</p>
                    @endif
                </div>
            </div>
        </div>
