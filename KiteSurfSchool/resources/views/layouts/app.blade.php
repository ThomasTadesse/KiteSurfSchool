...
<body class="bg-gradient-to-br from-blue-50 to-blue-100 min-h-screen">
    <nav class="bg-gradient-to-r from-blue-600 to-blue-800 p-6 flex justify-between items-center text-white shadow-lg">
        <div class="space-x-6">
            ...
            @auth
                <a href="/students" class="text-lg hover:text-blue-200 transition duration-200">Studenten</a>
                <a href="/instructors" class="text-lg hover:text-blue-200 transition duration-200">Instructeurs</a>
            @endauth
        </div>
        <div>
            @auth
                <a href="{{ route('profiel') }}" class="text-lg hover:text-blue-200 transition duration-200 mr-4">Profiel</a>
                <form action="{{ route('logout') }}" method="POST" class="inline">
                    @csrf
                    <button type="submit" class="text-lg hover:text-blue-200 transition duration-200">Log uit</button>
                </form>
            @else
                <a href="{{ route('login') }}" class="text-lg hover:text-blue-200 transition duration-200 mr-4">Login</a>
                <a href="{{ route('register') }}" class="text-lg hover:text-blue-200 transition duration-200">Registreren</a>
            @endauth
        </div>
    </nav>
